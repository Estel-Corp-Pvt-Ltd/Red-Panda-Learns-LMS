...
export async function getUsers(params) {
    ...
    const memFilters = {};
    let query = db.collection(COLLECTION.USERS);
    if (hasDateFilter) {
        if (params.role)
            memFilters.role = params.role;
        if (params.status)
            memFilters.status = params.status;
        if (params.email)
            memFilters.email = params.email;
        if (params.fromDate)
    ...
    if (memFilters.role)
        users = users.filter((u) => u.role === memFilters.role);
    if (memFilters.status)
        users = users.filter((u) => u.status === memFilters.status);
    if (memFilters.email)
        users = users.filter((u) => u.email === memFilters.email);
