...
import type { UserSnapshot, UsersState } from "./types.js";
...
export async function generateUsersState(): Promise<UsersState> {
  const snapshot = await db.collection(COLLECTION.USERS).get();

  const users: UserSnapshot[] = snapshot.docs.map((doc) => {
    const d = doc.data();
    return {
      id: doc.id,
      email: d.email ?? "",
