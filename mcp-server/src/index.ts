...
server.tool("ping", "Health check - returns pong", {}, async () => ({
  content: [
    { type: "text", text: JSON.stringify({ status: "ok", timestamp: new Date().toISOString() }) },
  ],
}));
...
registerTool(
  "get_users",
  "Get users from Firestore with optional filtering by role, status, or email",
  getUsersSchema,
  getUsers
);
registerTool(
  "get_courses",
  "Get courses from Firestore with optional filtering by status, instructor, or pricing model",
  getCoursesSchema,
  getCourses
);
registerTool(
  "get_enrollments",
  "Get enrollments from Firestore with optional filtering by userId, courseId, or status",
  getEnrollmentsSchema,
  getEnrollments
);
registerTool(
  "get_orders",
  "Get orders from Firestore with optional filtering by userId or status",
  getOrdersSchema,
  getOrders
);
registerTool(
...
registerTool(
  ...
  "Get full course curriculum structure with topics, lessons (with type, duration), and assignments (with points, deadlines).",
  getCourseCurriculumSchema,
  getCourseCurriculum
);
registerTool(
  "get_submissions",
  "Get assignment submissions with optional filtering by assignmentId, studentId, courseId, or ungraded only.",
  getSubmissionsSchema,
  getSubmissions
);
registerTool(
  "get_quiz_submissions",
  "Get quiz submissions for a quiz with scores, pass/fail status, and summary statistics.",
  getQuizSubmissionsSchema,
  getQuizSubmissions
);
registerTool(
  "get_coupons",
  "Get coupons from Firestore with optional filtering by status or code.",
  getCouponsSchema,
  getCoupons
);
registerTool(
  "get_bundles",
  "Get course bundles from Firestore with optional filtering by status or pricing model.",
  getBundlesSchema,
  getBundles
);
registerTool(
  "get_announcements",
  "Get announcements with optional filtering by scope, courseId, organizationId, or status.",
  getAnnouncementsSchema,
  getAnnouncements
);
registerTool(
  "get_banners",
  "Get banners with optional filtering by status.",
  getBannersSchema,
  getBanners
);
registerTool(
  "get_forum_channels",
  "Get forum channels for a course.",
  getForumChannelsSchema,
  getForumChannels
);
registerTool(
  "get_forum_messages",
  "Get messages from a forum channel with optional status filtering.",
  getForumMessagesSchema,
  getForumMessages
);
registerTool(
  "get_learning_progress",
  "Get student learning progress with optional filtering by userId or courseId.",
  getLearningProgressSchema,
  getLearningProgress
);
registerTool(
  "get_platform_stats",
  "Get comprehensive platform statistics including user, course, enrollment, and order counts with breakdowns.",
...
const refreshCourses = () => {
  refreshCoursesState().catch(() => {});
};

registerTool(
  "create_course",
  "Create a new draft course in Firestore with title, description, pricing, and instructor info. Returns the generated courseId and slug.",
  createCourseSchema,
  createCourse,
  refreshCourses
);
registerTool(
  "update_course",
  "Update any course fields by courseId. Supports title, description, slug, prices, status, tags, mode, certificate settings, and all toggles.",
  updateCourseSchema,
  updateCourse,
  refreshCourses
);
registerTool(
  "publish_course",
  "Publish a draft course. Validates the course has at least 1 topic with 1 lesson before publishing.",
  publishCourseSchema,
  publishCourse,
  refreshCourses
);
registerTool(
  "add_topic_to_course",
  "Add a new topic (section) to a course's curriculum. Returns the generated topicId.",
  addTopicToCourseSchema,
  addTopicToCourse,
  refreshCourses
);
registerTool(
  "create_lesson",
  "Create a new lesson in a course topic. Creates the lesson document and adds it to the topic's items array.",
  createLessonSchema,
  createLesson,
  refreshCourses
);
registerTool(
  "update_lesson",
  "Update lesson fields by lessonId. Also syncs title changes to the course curriculum.",
  updateLessonSchema,
  updateLesson,
  refreshCourses
);
registerTool(
  "delete_lesson",
  "Delete a lesson and remove its reference from the course curriculum. Also deletes associated attachments.",
  deleteLessonSchema,
  deleteLesson,
  refreshCourses
);
registerTool(
  "create_assignment",
  "Create a new assignment in a course topic. Creates the assignment document and adds it to the topic's items array.",
...
registerTool(
  "create_quiz",
  "Create a new quiz for a course with scheduling, duration, and passing criteria. Questions can be added via the admin dashboard.",
  createQuizSchema,
  createQuiz
);
registerTool(
  "scaffold_course",
  "Create a complete course skeleton from a structured input with topics, lessons, and assignments in a single call.",
  scaffoldCourseSchema,
  scaffoldCourse,
  refreshCourses
);
registerTool(
  "duplicate_course",
  "Duplicate an existing course with a new ID, slug, and DRAFT status. Copies all content including topics, lessons, and assignments.",
...
registerTool(
  "reorder_topics",
  "Reorder topics within a course and/or reorder items (lessons/assignments) within topics. Provide topicOrder and/or itemOrders.",
  reorderTopicsSchema,
  reorderTopics,
  refreshCourses
);
registerTool(
  "grade_submission",
  "Grade an assignment submission by submissionId. Sets marks and optional feedback. Validates marks against assignment maximum.",
...
const refreshUsers = () => {
  refreshUsersState().catch(() => {});
};

registerTool(
  "create_user",
  "Create a new user with Firebase Auth account and Firestore document. Supports role and organization assignment.",
  createUserSchema,
  createUser,
  refreshUsers
);
registerTool(
  "update_user",
  "Update user fields by userId. Supports name, role, status, and organization changes.",
  updateUserSchema,
  updateUser,
  refreshUsers
);
...
const refreshEnrollments = () => {
  refreshEnrollmentsState().catch(() => {});
};

registerTool(
  "enroll_student",
  "Enroll a student in a course by userId or email. Creates enrollment and learning progress documents. Rejects duplicate active enrollments.",
  enrollStudentSchema,
  enrollStudent,
  refreshEnrollments
);
registerTool(
  ...
  "Enroll multiple students in a course by email. Returns success/failure counts and details for each email.",
...
registerTool(
  "drop_enrollment",
  "Drop a student's enrollment by changing status to DROPPED. Requires userId and courseId.",
...
registerTool(
  "create_coupon",
  "Create a new discount coupon with code, percentage, expiry, and optional course/bundle linking.",
  createCouponSchema,
  createCoupon
);
registerTool(
  "update_coupon",
  "Update coupon fields by couponId. Supports code, discount, expiry, usage limit, status, and linked items.",
  updateCouponSchema,
  updateCoupon
);
...
registerTool(
  "create_bundle",
  "Create a new course bundle with auto-calculated pricing from included courses.",
  createBundleSchema,
  createBundle
);
registerTool(
  "update_bundle",
  "Update bundle fields by bundleId. Supports title, pricing, status, tags, and more.",
  updateBundleSchema,
  updateBundle
);
registerTool(
  "delete_bundle",
  "Delete a course bundle by bundleId.",
  deleteBundleSchema,
  deleteBundle
);
...
registerTool(
  "create_announcement",
  "Create a new announcement with scope (GLOBAL, COURSE, or ORGANIZATION).",
  createAnnouncementSchema,
  createAnnouncement
);
registerTool(
  "update_announcement",
  "Update announcement fields by announcementId.",
...
registerTool(
  "create_banner",
  "Create a new display banner with CTA, gradient colors, and course targeting.",
  createBannerSchema,
  createBanner
);
registerTool(
  "update_banner",
  "Update banner fields by bannerId. Supports title, CTA, image, status, and targeting.",
  updateBannerSchema,
  updateBanner
);
...
registerTool(
  ...
  "Create a new forum channel for a course with optional moderation.",
  createForumChannelSchema,
  createForumChannel
);
registerTool(
  "create_forum_message",
  "Post a message in a forum channel. Supports threaded replies and moderation.",
...
server.tool(
  "refresh_state",
  "Regenerate all Central Brain state files (courses, users, enrollments, orders) from Firestore",
  {},
  async () => {
    try {
...
server.tool(
  "get_system_status",
  "Get current system status including last refresh time, collection counts, and server uptime",
  {},
  async () => {
    try {
...
async function main() {
  ...
  console.error("RedPanda Learns MCP server v2.0.0 running on stdio");
}

main().catch((error) => {
  console.error("Failed to start RedPanda Learns MCP server:", error);
  process.exit(1);
});
