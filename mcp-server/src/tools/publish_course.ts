...
export async function publishCourse(params: { courseId: string }) {
  ...
  const hasContent = topics.some(
    (t: any) => t.items && t.items.length > 0
  );

  if (topics.length === 0 || !hasContent) {
    throw new Error(
      `Cannot publish course "${data.title}": it must have at least 1 topic with 1 lesson or assignment`
    );
  }
