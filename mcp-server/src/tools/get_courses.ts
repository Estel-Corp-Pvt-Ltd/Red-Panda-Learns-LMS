...
export async function getCourses(params: {
  ...
  let query: FirebaseFirestore.Query = db.collection(COLLECTION.COURSES);

  if (hasDateFilter) {
    if (params.status) memFilters.status = params.status;
    if (params.instructorId) memFilters.instructorId = params.instructorId;
    if (params.pricingModel) memFilters.pricingModel = params.pricingModel;
    ...
    if (params.instructorId) query = query.where("instructorId", "==", params.instructorId);
    if (params.pricingModel) query = query.where("pricingModel", "==", params.pricingModel);
  }
  ...
  if (memFilters.status) courses = courses.filter((c) => c.status === memFilters.status);
  if (memFilters.instructorId) courses = courses.filter((c) => c.instructorId === memFilters.instructorId);
  if (memFilters.pricingModel) courses = courses.filter((c) => c.pricingModel === memFilters.pricingModel);
