...
export async function getEnrollments(params: {
  ...
  let query: FirebaseFirestore.Query = db.collection(COLLECTION.ENROLLMENTS);

  if (hasDateFilter) {
    // Apply date range in Firestore, everything else in-memory
    if (params.userId) memFilters.userId = params.userId;
    if (params.courseId) memFilters.courseId = params.courseId;
    ...
    if (params.courseId) query = query.where("courseId", "==", params.courseId);
  ...
  if (memFilters.userId) enrollments = enrollments.filter((e) => e.userId === memFilters.userId);
  if (memFilters.courseId) enrollments = enrollments.filter((e) => e.courseId === memFilters.courseId);
  if (memFilters.status) enrollments = enrollments.filter((e) => e.status === memFilters.status);
