...
async function createCourseManualAnnouncementHandler(req: Request, res: Response) {
  try {
    ...
    const { title, body, status, courseId } = req.body;
...
export const createCourseManualAnnouncement = onRequest(
  { region: "us-central1" },
  withMiddleware(corsMiddleware, authMiddleware, createCourseManualAnnouncementHandler)
);