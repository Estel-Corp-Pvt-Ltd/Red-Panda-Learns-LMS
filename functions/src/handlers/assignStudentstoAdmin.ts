...
async function assignStudentsToAdminHandler(req: Request, res: Response) {
  try {
    ...
    if (!adminId || !Array.isArray(studentIds) || studentIds.length === 0) {
      res.status(400).json({
        error: "Missing or invalid required fields: adminId, studentIds (array)",
      });
      return;
    }
...
export const assignStudentsToAdmin = onRequest(
  { region: "us-central1" },
  withMiddleware(corsMiddleware, authMiddleware, assignStudentsToAdminHandler)
);
