...
async function unassignStudentsToAdminHandler(req: Request, res: Response) {
  try {
    ...
    const assigned = await assignStudentsService.unassignStudentsFromAdmin(
      adminId,
      studentIds
    );
...
export const bulkUnassignStudentsFromAdmin = onRequest(
  { region: "us-central1" },
  withMiddleware(corsMiddleware, authMiddleware, unassignStudentsToAdminHandler)
);
