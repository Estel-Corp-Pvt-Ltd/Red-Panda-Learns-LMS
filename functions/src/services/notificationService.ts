...
export const notificationService = {
  ...
  async createNotification(data: {
    ...
    try {
      ...
      const pausedAdminsResult =
        await reminderService.checkIsReminderPausedForAssignmentforThisAdmin(data.assignmentId);

      // Get the array of paused admin IDs (empty array if none or on error)
      const pausedAdminIds: string[] =
        pausedAdminsResult.success && pausedAdminsResult.data ? pausedAdminsResult.data : [];
  ...
  async scheduleReminder(id: string) {
    try {
      ...
      const pausedAdminsResult =
        await reminderService.checkIsReminderPausedForAssignmentforThisAdmin(assignmentId);
  ...
  async sendInitialEmail(id: string, shouldScheduleReminder = true) {
    try {
      ...
      const evalLink = `https://RedPanda Learns.ai/admin/submissions?submissionId=${submissionId}`;
