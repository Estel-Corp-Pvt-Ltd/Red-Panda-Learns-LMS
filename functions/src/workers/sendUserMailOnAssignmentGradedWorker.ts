...
export const sendUserMailOnAssignmentGradedWorker = onMessagePublished(
  {
  ...
  async (event) => {
    try {
      ...
      const response = await fetch("https://api.brevo.com/v3/smtp/email", {
        ...
        body: JSON.stringify({
          sender: {
            name: "RedPanda Learns", // Sender's name (you can change it)
            email: "no_reply@RedPanda Learns.com", // Sender's email
          },
