...
export const sendComplaintRedressalMailWorker = onMessagePublished(
  ...
  async (event) => {
    ...
    try {
      ...
      const { to, subject, message, complaintId, status, isInternal } = payload;
      ...
      const complaintSnap = await db.collection("Complaints").doc(complaintId).get();
      ...
      const sender = {
        name: "RedPanda Learns Support",
        email: "no_reply@RedPanda Learns.com",
      };
      ...
      const textContent = message || (html ? html.replace(/<[^>]+>/g, "") : "");
      ...
      if (!res.ok) {
        ...
        throw new Error(`Brevo API failed (${res.status}): ${errorText}`);
      }
