...
| P0 | Prevent orphaned Firebase Auth users when signup fails after `firebaseCreateUser()` — if `updateProfile()` or Firestore doc creation fails, delete the Auth user, sign out, and clear client storage. Treat email verification failure separately (resend support or full cleanup). | Open |
