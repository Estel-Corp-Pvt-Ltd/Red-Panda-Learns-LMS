...
class UserService {
  ...
  async doesEmailExists(email: string): Promise<Result<boolean>> {
    try {
      const usersRef = collection(db, COLLECTION.USERS);
      const q = query(usersRef, where("email", "==", email));
      // Execute query
  ...
  async getFilteredUsers(
    ...
    try {
      ...
      if (filters && filters.length > 0) {
        queryRef = query(usersRef, ...filters.map((f) => where(f.field as string, f.op, f.value)));
      }
   ...
   */
  async changeUserStatus(uid: string, newStatus: UserStatus): Promise<Result<void>> {
    try {
      const userRef = doc(db, COLLECTION.USERS, uid);
      await updateDoc(userRef, {
        status: newStatus,
  ...
  async getUsersByRole(
    ...
    return this.getUsers([{ field: "role", op: "==", value: role }], options);
  }

  /**
   * Retrieves users belonging to a specific organization, optionally filtered by role.
   */
  async getUsersByOrganization(
    ...
    const filters: { field: keyof User; op: WhereFilterOp; value: any }[] = [
      { field: "organizationId", op: "==", value: organizationId },
    ];
    if (roleFilter) {
      filters.push({ field: "role", op: "==", value: roleFilter });
    }
    return this.getUsers(filters, options);
  }

  /**
   * Retrieves all user IDs belonging to an organization with a given role (non-paginated).
   */
  async getOrganizationUserIds(organizationId: string, role: string): Promise<Result<string[]>> {
    try {
      const usersRef = collection(db, COLLECTION.USERS);
      const q = query(
        usersRef,
      ...
      logError("UserService.getOrganizationUserIds", error);
      return fail("Failed to fetch organization user IDs");
    }
  }
   ...
   */
  async getUidListForEmails(emailCsv: string): Promise<Result<(string | null)[]>> {
    try {
   ...
   */
  async getEmailsForUidList(uidList: string[]): Promise<Result<(string | null)[]>> {
    try {
  ...
  async upsertFcmToken(uid: string, newToken: FcmToken): Promise<Result<void>> {
    try {
      const userRef = doc(db, COLLECTION.USERS, uid);
      ...
      const tokenExists = existingTokens.some((t) => t.token === newToken.token);

      const updatedTokens = tokenExists
        ? existingTokens.map((t) =>
            t.token === newToken.token ? { ...t, updatedAt: new Date() } : t
          )
