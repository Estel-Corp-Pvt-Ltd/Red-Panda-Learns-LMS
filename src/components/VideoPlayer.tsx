...
type LmsVideoPlayerProps = {
  ...
  onEnded?: () => void;
  className?: string;
  onDuration?: (durationInSeconds: number) => void;
...
export default function VideoPlayer({
  ...
  useEffect(() => {
    const getDuration = () => {
      if (playerRef.current && playerRef.current.plyr) {
        ...
        if (!plyrInstance._hasTimeUpdateListener) {
          plyrInstance.on("timeupdate", () => {
            onTimeUpdate?.(plyrInstance.currentTime);
          });
          plyrInstance._hasTimeUpdateListener = true;
        }
      }
    };

    const intervalId = setInterval(getDuration, 100);

    return () => clearInterval(intervalId);
  }, [url, onDuration, onTimeUpdate]);
  const handleEnded = () => {
  ...
  if (!videoSource || error) {
    return (
      <div
        className={`relative w-full aspect-video rounded-2xl overflow-hidden bg-gray-100 flex items-center justify-center ${className}`}
      >
