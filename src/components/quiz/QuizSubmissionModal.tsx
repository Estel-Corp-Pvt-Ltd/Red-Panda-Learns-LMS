...
const QuizSubmissionModal: React.FC<QuizSubmissionModalProps> = ({
  ...
  const [resettingSubmission, setResettingSubmission] = useState<string | null>(
    null
  );
  ...
  const calculateTimeSpent = (submission: QuizSubmission): number => {
    ...
    const submitted = new Date(
      formatDateTime(submission.submittedAt || submission.lastSavedAt)
    );
  ...
  const getStatusBadge = (status: QuizSubmissionStatus, passed?: boolean) => {
    ...
    const config =
      statusConfig[status] ||
      statusConfig[QUIZ_SUBMISSION_STATUS.NOT_SUBMITTED];
  ...
  const resetQuizForStudent = async (submission: QuizSubmission) => {
    if (
      !window.confirm(
        `Are you sure you want to reset the quiz for ${submission.userName}? This will delete their current submission and allow them to start fresh.`
      )
    ) {
      return;
    }
  ...
  const exportToCSV = () => {
    ...
    const getResultStatus = (submission: QuizSubmission): string => {
      if (submission.status !== QUIZ_SUBMISSION_STATUS.SUBMITTED)
    ...
    a.href = url;
    a.download = `quiz-results-${quiz.title}-${
  ...
  const stats = {
    ...
    submitted: quizSubmissions.filter(
      (s) => s.status === QUIZ_SUBMISSION_STATUS.SUBMITTED
    ).length,
    inProgress: quizSubmissions.filter(
      (s) => s.status === QUIZ_SUBMISSION_STATUS.IN_PROGRESS
    ).length,
    attempted: quizSubmissions.filter(
      (s) =>
  ...
  const colorClasses = {
    primary:
      "bg-primary/10 text-primary border-primary/30 dark:bg-primary/20 dark:border-primary/50 dark:text-primary",
    secondary:
      "bg-secondary/10 text-secondary border-secondary/30 dark:bg-secondary/20 dark:border-secondary/50 dark:text-secondary",
    accent:
      "bg-accent/10 text-accent border-accent/30 dark:bg-accent/20 dark:border-accent/50 dark:text-accent",
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          {[
            { label: "Total Students", value: stats.total, color: "primary" },
            { label: "Attempted", value: stats.attempted, color: "accent" },
            { label: "Submitted", value: stats.submitted, color: "primary" },
            { label: "Passed", value: stats.passed, color: "accent" },
            { label: "In Progress", value: stats.inProgress, color: "primary" },
          ].map(({ label, value, color }) => (
            <div
              key={label}
              className={`p-4 rounded-lg border rounded-lg ${colorClasses[color]}`}
            >
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            <div className="flex justify-center items-center py-10">
              <div className="animate-spin h-8 w-8 rounded-full border-b-2 border-accent" />
            </div>
