    ...
    try {
      ...
      rows.forEach((row: any, index: number) => {
        ...
        if (!/^\d{6}$/.test(password)) {
          invalidRows.push(
            `Row ${rowNumber}: Password must be a 6-digit number`
          );
          return;
        }
      ...
      const res = await fetch(`${BACKEND_URL}/enrollStudentsInBulk`, {
        method: "POST",
        headers: {
    ...
    <AdminLayout>
      <Card className="max-w-xl mx-auto mt-6">
        <CardHeader>
          ...
          <CardDescription>
            Upload an Excel file to enroll students into courses. It should
