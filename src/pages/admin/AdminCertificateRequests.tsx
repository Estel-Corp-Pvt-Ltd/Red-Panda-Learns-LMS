...
const AdminCertificateRequests = () => {
  ...
  const loadRequests = async () => {
    ...
    const result = await certificateRequestService.getCertificateRequests(
      {
        limit: itemsPerPage,
  ...
  const handlePreviousPage = () => {
    if (!requests.hasPreviousPage || isLoading || pagination.currentPage <= 1) return;
  ...
  const approveRequest = async (requestId: string) => {
    ...
    const result = await certificateRequestService.approveCertificateRequest(requestId, adminUid);
  ...
  const rejectRequest = async (requestId: string) => {
    ...
    const result = await certificateRequestService.rejectCertificateRequest(requestId, adminUid);
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {requests.totalCount > 0 && ` (Page ${pagination.currentPage})`}
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <>
            ...
            {requests.data.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">No certificate requests</div>
            ) : (
              <>
                <Table>
                  <TableHeader>
                ...
                <div className="flex justify-between items-center pt-4">
                  <div className="text-sm text-muted-foreground">
                    Page {pagination.currentPage} of {Math.ceil(requests.totalCount / itemsPerPage)}
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      disabled={
                        !requests.hasPreviousPage || isLoading || pagination.currentPage <= 1
                      }
                    >
