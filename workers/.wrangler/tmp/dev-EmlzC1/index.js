...
// .wrangler/tmp/bundle-U3tgw6/strip-cf-connecting-ip-header.js
...
// src/lib/service-account.ts
var _cachedToken = null;
var _cachedServiceEmail = null;
...
async function getAccessToken(serviceAccountJson) {
  ...
  if (_cachedToken && _cachedServiceEmail === key.client_email && Date.now() < _cachedToken.expiresAt - 6e4) {
  ...
  _cachedToken = { token: data.access_token, expiresAt: Date.now() + 3600 * 1e3 };
...
var SERVER_TIMESTAMP = { __type: "serverTimestamp" };
...
function toFSValue(v) {
  ...
  if (Array.isArray(v)) {
    return {
      arrayValue: {
        values: v.map(toFSValue).filter((x) => x !== null)
      }
    };
  }
  if (typeof v === "object") {
...
__name(toFSValue, "toFSValue");
function isSentinel(v) {
  return typeof v === "object" && v !== null && "__type" in v && ["serverTimestamp", "deleteField", "increment"].includes(v.__type);
}
...
__name(fromFSValue, "fromFSValue");
...
__name(fromFirestoreDoc, "fromFirestoreDoc");
function docIdFromName(name) {
  return name.split("/").pop() ?? "";
}
__name(docIdFromName, "docIdFromName");
function extractTransforms(data, prefix = "") {
  const transforms = [];
  for (const [k, v] of Object.entries(data)) {
    ...
    if (isSentinel(v)) {
      ...
      if (s.__type === "serverTimestamp") {
        ...
        transforms.push({
          fieldPath: path,
          transform: Number.isInteger(val) ? { increment: { integerValue: String(val) } } : { increment: { doubleValue: val } }
        });
      }
    } else if (typeof v === "object" && v !== null && !Array.isArray(v) && !(v instanceof Date)) {
      transforms.push(...extractTransforms(v, path));
    }
  }
  return transforms;
}
__name(extractTransforms, "extractTransforms");
var Firestore = class {
  baseUrl;
  tokenFn;
  ...
  async query(collection, where = [], orderBy, limit) {
  ...
  async runTransaction(fn) {
    ...
    const { transaction: txId } = await beginResp.json();
...
var FirestoreTransaction = class {
  constructor(baseUrl, txId, headers) {
...
__name(FirestoreTransaction, "FirestoreTransaction");
function fsOp(op) {
  const map = {
    "==": "EQUAL",
    "!=": "NOT_EQUAL",
...
async function getJWKs() {
  if (_jwkCache && Date.now() - _jwkCache.fetchedAt < 3600 * 1e3) {
...
async function getAuthUserByEmail(email, projectId, serviceAccountJson) {
...
__name(getAuthUserByEmail, "getAuthUserByEmail");
async function createAuthUser(opts, projectId, serviceAccountJson) {
  const token = await getAccessToken(serviceAccountJson);
  const resp = await fetch(
...
var RazorpayClient = class {
  ...
  async createOrder(opts) {
    return this.request("POST", "/orders", opts);
  }
};
__name(RazorpayClient, "RazorpayClient");
...
async function sendPaymentConfirmation(data, brevoApiKey) {
...
__name(sendPaymentConfirmation, "sendPaymentConfirmation");
...
function db(env2) {
  return new Firestore(env2.FIREBASE_PROJECT_ID, env2.FIREBASE_SERVICE_ACCOUNT_JSON);
}
__name(db, "db");
async function generateOrderId(firestore) {
...
async function getCourseById(firestore, courseId) {
  ...
  return {
    ...
    slug: data.slug,
    originalAmount: data.originalAmount ?? data.salePrice ?? 0
  };
}
__name(getCourseById, "getCourseById");
async function getBundleById(firestore, bundleId) {
...
__name(getBundleById, "getBundleById");
async function getItemsDetails(firestore, items) {
  const itemsDetails = [];
  await Promise.all(
...
__name(getItemsDetails, "getItemsDetails");
async function enrollUser(firestore, user, items, orderId) {
  if (!items.length)
    return [];
...
__name(enrollUser, "enrollUser");
function buildEnrollmentWrite(firestore, enrollmentId, data) {
  const { SERVER_TIMESTAMP: _st, ...rest } = {};
  const baseUrl = firestore.baseUrl;
  const docPath = baseUrl.replace("https://firestore.googleapis.com/v1/", "");
  const fields = {};
  for (const [k, v] of Object.entries(data)) {
    if (typeof v !== "object" || v === null || !("__type" in v)) {
      if (v !== null && v !== void 0)
        fields[k] = toSimpleFS(v);
    }
  }
  return {
    update: {
...
__name(buildEnrollmentWrite, "buildEnrollmentWrite");
function buildLPWrite(firestore, lpId, userId, courseId) {
  const baseUrl = firestore.baseUrl;
  const docPath = baseUrl.replace("https://firestore.googleapis.com/v1/", "");
  return {
    update: {
...
__name(buildLPWrite, "buildLPWrite");
function toSimpleFS(v) {
  if (v === null)
    return { nullValue: null };
  if (typeof v === "string")
    return { stringValue: v };
  ...
  if (typeof v === "object") {
    ...
    for (const [k, val] of Object.entries(v)) {
      if (val !== void 0)
        fields[k] = toSimpleFS(val);
    }
    return { mapValue: { fields } };
  }
  return { stringValue: String(v) };
}
__name(toSimpleFS, "toSimpleFS");
function serializeOrder(id, data) {
  const fmt = /* @__PURE__ */ __name((v) => {
    if (!v)
      return "N/A";
...
__name(serializeOrder, "serializeOrder");
function mapActionToBreakdown(category, action) {
...
__name(mapActionToBreakdown, "mapActionToBreakdown");
app.get(
  "/ordersHealthCheck",
  (c) => c.json({ success: true, data: { status: "healthy", service: "lms-api" } })
);
app.get("/getOrders", async (c) => {
  ...
  const { status, userId, email, provider, startDate, endDate, limit } = c.req.query();
  const where = [];
  if (status)
  ...
  if (email)
    where.push({ field: "userEmail", op: "==", value: email.toLowerCase() });
  if (provider)
    where.push({ field: "provider", op: "==", value: provider });
  ...
  return c.json({ success: true, data: orders, meta: { count: orders.length, timestamp: (/* @__PURE__ */ new Date()).toISOString() } });
});
app.get("/getOrderById", async (c) => {
  ...
  return c.json({ success: true, data: serializeOrder(orderId, data), meta: { timestamp: (/* @__PURE__ */ new Date()).toISOString() } });
});
app.get("/getOrderStats", async (c) => {
  ...
  return c.json({ success: true, data: counts, meta: { timestamp: (/* @__PURE__ */ new Date()).toISOString() } });
});
  ...
  if (promoCode) {
    ...
    if (coupons.length > 0) {
      ...
      if (existingUsage.length === 0) {
        const pct = couponData.discountPercentage ?? 0;
        const linkedCourseIds = couponData.linkedCourseIds ?? [];
        const linkedBundleIds = couponData.linkedBundleIds ?? [];
        let restUsages = (couponData.usageLimit ?? 0) - (couponData.totalUsed ?? 0);
        for (const item of itemsDetails) {
          if (restUsages <= 0)
            break;
          if (item.itemType === ENROLLED_PROGRAM_TYPE.COURSE && linkedCourseIds.includes(item.itemId) || item.itemType === ENROLLED_PROGRAM_TYPE.BUNDLE && linkedBundleIds.includes(item.itemId)) {
...
async function getOrderByProviderId(firestore, providerOrderId) {
  const docs = await firestore.query(
    COLLECTION.ORDERS,
...
__name(getOrderByProviderId, "getOrderByProviderId");
async function handlePaymentCaptured(firestore, entity) {
...
__name(handlePaymentCaptured, "handlePaymentCaptured");
async function handlePaymentFailed(firestore, entity) {
  const order = await getOrderByProviderId(firestore, entity.order_id);
  if (!order)
    return;
  const txId = `tnx_${crypto.randomUUID()}`;
...
async function handleOrderPaid(firestore, entity, env2) {
  ...
  await enrollUser(firestore, { id: userId, ...userData }, items, orderId);
  try {
    const enrolledItems = [];
    for (const item of items) {
      if (item.itemType === ENROLLED_PROGRAM_TYPE.COURSE) {
        const course = await getCourseById(firestore, item.itemId);
        if (course)
          enrolledItems.push({ itemType: "COURSE", itemId: item.itemId, name: course.title, slug: course.slug });
      } else if (item.itemType === ENROLLED_PROGRAM_TYPE.BUNDLE) {
        const bundle = await getBundleById(firestore, item.itemId);
        if (bundle)
          enrolledItems.push({ itemType: "BUNDLE", itemId: item.itemId, name: bundle.title, slug: bundle.slug });
      }
    }
...
__name(handleOrderPaid, "handleOrderPaid");
async function handleOrderExpired(firestore, entity) {
  const order = await getOrderByProviderId(firestore, entity.id);
  if (!order)
    return;
  const orderId = order.data.orderId ?? order.id;
...
__name(handleOrderExpired, "handleOrderExpired");
app.post("/enrollStudent", async (c) => {
  ...
  await enrollUser(firestore, { id: userId, ...userData }, itemsDetails, "Admin Enrollment");
...
app.post("/enrollFreeCourse", async (c) => {
  ...
  try {
    ...
    if (course.salePrice > 0)
      return c.json({ error: "Course is not free" }, 400);
    ...
    await enrollUser(firestore, { id: user.uid, ...userData }, itemsDetails, "Free Course Enrollment");
    return c.json({ success: true, items: itemsDetails });
  } catch (err) {
...
app.post("/addKarma", async (c) => {
  ...
  const points = rules[0].data.points;
...
// .wrangler/tmp/bundle-U3tgw6/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-U3tgw6/middleware-loader.entry.ts
